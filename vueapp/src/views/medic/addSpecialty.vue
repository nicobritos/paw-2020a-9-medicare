...
<div class="container w-100 h-100 d-flex flex-column justify-content-center align-items-center">
    ...
    <form:form modelAttribute="specialtyForm" class="addturn-form border p-5 rounded" action="${addSpecialtyUrl}"
        ...
        <div class="row justify-content-start">
            <h1 class="addturn-form-title"><spring:message code="AddSpecialty"/></h1>
        </div>

        <div class="form-group row">
            <div class="col">
                <label for="specialtyId"><spring:message code="Specialty"/></label>
            </div>
            <div class="col-8">
                <form:select class="form-control" name="specialtyId" id="specialtyId" path="specialtyId">
                    <c:forEach var="specialty" items="${specialties}">
                        <form:option value="${specialty.id}"><c:out value="${specialty.name}"/></form:option>
                    </c:forEach>
                </form:select>
            </div>
        </div>
